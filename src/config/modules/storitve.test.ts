...
describe('Roki, plačila in vezan denar', () => {
  ...
  it('"Ne vem" pade na najkonservativnejši delež sprostitve', () => {
    const unknown = run(terjatve, { unbilledWipEUR: 300_000, reducibleShare: 4 });
    const lowest = run(terjatve, { unbilledWipEUR: 300_000, reducibleShare: 0 });
    expect(pick(unknown, 'Sprostljiv kapital v nezaračunanem delu in terjatvah').valueEUR).toBe(
      pick(lowest, 'Sprostljiv kapital v nezaračunanem delu in terjatvah').valueEUR,
    );
  });
});
